...
- КРИТИЧЕСКИЙ БАГ (из тест-центра): на экране «История воды» статус-бар был непрозрачным —
  единственный экран в приложении, где корневой `Box` фона был `fillMaxWidth()` вместо
  `fillMaxSize()`, поэтому цвет `bg` не дотягивался до полной высоты экрана и не подкладывался
  под статус-бар как на остальных экранах. Поправлено на `fillMaxSize()`.
- Аватарка из Telegram не подтягивалась: `fetchProfilePhotoUrl` дергал `getUserProfilePhotos`
  РОВНО ОДИН раз, сразу в момент бинда — а сразу после «/start» Telegram иногда ещё не успевает
  зарегистрировать чат на своей стороне и отдаёт `ok:true` с пустым `photos: []`, хотя фото у
  аккаунта есть. Добавил ретраи (3 попытки с бэкоффом 0/900/1800мс) внутри самого фетча, плюс
  бэкфилл в `AppState.init` — если уже привязан, но `photoUrl == null`, повторяем попытку при
  каждом запуске приложения (закрывает случай, когда бинд был раньше, а фото так и не подтянулось).
- «Плашка» сегмент-тогглов (`SegmentedTabs` — Вода/Еда и Неделя/Месяц) уменьшена: 54dp -> 42dp
  высотой, меньше текст/иконки, компактнее паддинги — было слишком крупно и грубо.
- Экран «Питание» → вкладка «Вода» — редизайн:
  * Кольцо прогресса заменено на `LiquidVessel` (тот же «жидкий сосуд» с волной, что и у
    привычек на «План») — теперь вода реально «наливается» в круг, а не абстрактная дуга.
    Добавлен параметр `showIcon` в `LiquidVessel` (default true, старые вызовы не ломаются),
    чтобы крупный сосуд на 200dp не рисовал поверх текста ещё и гигантскую иконку.
  * Сетка быстрых кнопок заменена на новый компонент `AmountSlider` (components/AmountSlider.kt):
    таблетка-трек с белым ползунком, который можно тащить пальцем, значение показывается в
    пузыре-подсказке над ползунком (со скруглённым хвостиком) — по референсу со скрина.
    Под слайдером — кнопка «Добавить».
- versionCode 156->157, versionName "r157-water-redesign".
- ОГРАНИЧЕНИЯ / что НЕ сделано в этом заходе: глобальный проход по скруглениям остальных
  контейнеров в приложении (карточки/чипы/т.д.) не делал — тронул только новые элементы
  (слайдер, пузырь-подсказка, тогглы). Сборку локально не проверял — в этой среде нет доступа
  к интернету для Gradle/зависимостей, нужен тест на устройстве или CI.
