## r158-history-statusbar-fix (versionCode 158) — critical, from testing
- dletniev (первый центр тестирования, критически-опасно): «на экране истории воды статус-бар
  не прозрачный как на остальных экранах».
- ПРИЧИНА: `WaterHistoryScreen` — единственный settings-style экран, у которого корневой Box и
  скроллящийся Column были размечены только `Modifier.fillMaxWidth()`, БЕЗ высоты вообще. Все
  соседи (`BindingsScreen`, `NotificationsScreen`, `AppearanceScreen`, ...) используют
  `fillMaxSize()`. Без него фон экрана (`Letify.colors.bg`) не гарантированно растягивался до
  самого верха окна — из-за чего сквозь прозрачный статус-бар именно на этом экране просвечивал
  чёрный фон окна Activity по умолчанию, а не фон приложения.
- ФИКС: `fillMaxWidth()` -> `fillMaxSize()` на корневом `Box` и на скроллящемся `Column`,
  как везде. Добавлен импорт `fillMaxSize`.
- versionCode 157->158.

## r157-water-redesign (versionCode 157)
- dletniev: «не берётся авка из тг до сих пор» + «скругления контейнеров подкорректировать» +
  редизайн экрана питания/воды/еды.
- АВАТАРКА (реальный баг, не настройка приватности): `AppState.telegramUser` восстанавливается из
  SharedPreferences при каждом холодном старте, но фото резолвится только ОДИН раз — в момент самого
  байндинга (`BindingsScreen` → `state.fetchTelegramPhoto(...)` сразу после `PollResult.Found`). Если
  тот единственный запрос не удался или не успел (слабая сеть в момент байндинга, `getUserProfilePhotos`
  на секунду вернул пусто сразу после `/start`, процесс убит до второго round-trip) — `photoUrl`
  сохранялся `null` и НИЧЕГО больше не пыталось его перезапросить: при каждом следующем запуске
  грузился тот же `null` из диска. Фикс: добавлен `init {}` в `AppState` — если после восстановления
  из хранилища юзер привязан, но `photoUrl == null`, сразу дергаем `fetchTelegramPhoto(id)` ещё раз.
  Один лишний сетевой запрос при старте, если фото и так не приезжает — незаметно.
- ПЛАШКА (Вода/Еда + Неделя/Месяц): обе — один и тот же `SegmentedTabs`. Был 44dp трек + 5dp паддинг
  (~54dp) с сплошной заливкой на всю ширину — «крупная, стрёмная». Ужат до 36dp трека, 3dp паддинга,
  мельче иконка/шрифт — компонент общий, чинится сразу в обоих местах.
- КОЛЬЦО ВОДЫ → «стакан» (LiquidVessel): вместо `ProgressRing` на вкладке «Вода» теперь тот же
  жидкостный сосуд, что и на «План» (пер референс — «как будто стакан, который наполняется»),
  публичный `LiquidVessel`/`rememberDeviceTilt` из `PlanScreen.kt` переиспользованы напрямую (тот же
  пакет `ui.screens`), а не написаны с нуля.
- БЫСТРЫЕ КНОПКИ → слайдер: 2x2 грид фиксированных объёмов заменён на `WaterAmountPicker` —
  перетаскиваемая «пимпочка» на пилюле-треке (50–750 мл, шаг 25), пузырь-подсказка с текущим
  значением над пальцем (по референс-скрину), кнопка «Добавить» под треком.
- versionCode 156->157, versionName "r157-water-redesign".
- ВАЖНО: собрать здесь не смог — песочница этой сессии режет сеть до узкого allowlist без
  maven.google.com/services.gradle.org, поэтому `./gradlew` падает на самой загрузке дистрибутива.
  Изменения непроверены компилятором — нужно собрать через GitHub Actions (`.github/workflows/build.yml`)
  или Android Studio перед установкой на устройство.
